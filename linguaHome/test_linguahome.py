...
# Add parent directory to path for linguaHome imports
...
def test_imports():
    ...
    from linguaHome import __version__
    print(f"✅ LinguaHome version: {__version__}")
    
    from linguaHome.config import DEVICE_MAP, ROOMS
    print(f"✅ Config: {len(DEVICE_MAP)} devices, {len(ROOMS)} rooms")
    
    from linguaHome.agent.memory import MemoryStore
    print("✅ MemoryStore")
    
    from linguaHome.agent.context import ContextBuilder, SYSTEM_PROMPT
    print(f"✅ ContextBuilder (System Prompt: {len(SYSTEM_PROMPT)} chars)")
    
    from linguaHome.agent.code_executor import CodeExecutor
    print("✅ CodeExecutor")
    
    from linguaHome.mock_sensors import Sensors, ZWaveHomeActuator
    print("✅ Mock Sensors")
